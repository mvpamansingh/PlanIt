...
class LowPriorityViewModel():ViewModel(), KoinComponent {

    private val repository :RoomRepository by inject()
    private val _state = MutableStateFlow(LowPriorityState())
    ...
    fun OnEvent(events: LowPriorityEvents)
    {
        when(events)
        {
            is LowPriorityEvents.OnTaskClicked -> {
                TODO()
            }
            is LowPriorityEvents.OnTaskStatusChanged -> {
                updateTaskStatus(events.taskId, events.isCompleted)
            }

            is LowPriorityEvents.SearchStateChanged -> {
        ...
        viewModelScope.launch {
            ...
            try {
                ...
                repository.getTodayTasksByPriority(startOfDay, endOfDay).collect { tasks ->
                    val highPriorityTasks = tasks.filter { it.priority == Priority.LOW }
