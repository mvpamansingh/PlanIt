...
class MediumPriorityViewModel():ViewModel(), KoinComponent {

    private val repository :RoomRepository by inject()
    private val _state = MutableStateFlow(MediumPriorityState())
    ...
    fun OnEvent(events: MediumPriorityEvents)
    {
        when(events)
        {
            is MediumPriorityEvents.OnTaskClicked -> {
                TODO()
            }
            is MediumPriorityEvents.OnTaskStatusChanged -> {
                updateTaskStatus(events.taskId, events.isCompleted)
            }

            is MediumPriorityEvents.SearchStateChanged -> {
        ...
        viewModelScope.launch {
            ...
            try {
                ...
                repository.getTodayTasksByPriority(startOfDay, endOfDay).collect { tasks ->
                    val highPriorityTasks = tasks.filter { it.priority == Priority.MEDIUM }
